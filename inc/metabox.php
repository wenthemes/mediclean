...
if ( ! function_exists( 'mediclean_save_theme_settings_meta' ) ) :
	...
	function mediclean_save_theme_settings_meta( $post_id, $post ) {
		...
		// Verify nonce.
		if ( ! isset( $_POST['mediclean_theme_settings_meta_box_nonce'] ) || ! wp_verify_nonce( $_POST['mediclean_theme_settings_meta_box_nonce'], basename( __FILE__ ) ) ) {
			return;
		}
		...
		if ( ! array_filter( $_POST['mediclean_theme_settings'] ) ) {
			// No value.
			...
			foreach ( $_POST['mediclean_theme_settings'] as $mk => $mv ) {
...
add_action( 'save_post', 'mediclean_save_theme_settings_meta', 10, 3 );
